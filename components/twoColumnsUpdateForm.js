...
const TwoColumnsUpdateForm = ({id, uIstate, cb, type, administrationId, setId}) => {
  ...
  const handleSubmit = (e, id) => {
    e.preventDefault();
    const employers = update(uIstate, id, value, type, administrationId);
    cb(employers);
  };
  return(
    <>
            ...
            <tbody>
              <tr>
                <td className='vertical-align-middle w-1056'>
                  {type === 'email' && <input className='min-w-0' ref={inputEl} onChange={(e) => setValue(e.target.value)} value={value} type='email'></input>}
                  {type === 'phone_code' && <input className='min-w-0' ref={inputEl} onChange={(e) => setValue(e.target.value)} value={value} type='number'></input>}
                  {type === 'location' && <input className='min-w-0' ref={inputEl} onChange={(e) => setValue(e.target.value)} value={value}></input>}
                </td>
                <td>
...
const update = (state, id, value, type, administrationId) => {
  const searchValue = id.endsWith('2') ? 'email' : 'department';
