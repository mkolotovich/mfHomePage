...
const AddForm = ({id, uIstate, cb, type, administrationId, setId}) => {
  ...
  const handleSubmit = (e, id) => {
    e.preventDefault();
    const employers = addAuthorityUser(uIstate, values, id, type, administrationId);
    cb([employers]);
  };
  return(
    <tr>
...
const findMaxIndex = (uIstate, administrationId) => {
  let index = 0;
...
const addAuthorityUser = (uIstate, values, id, type, administrationId) => {
  const [position, fullName, workPhone, mobilePhone] = values;
  const index = uIstate.findIndex((el) => el._id === administrationId);
  const accumalator = uIstate.filter((el) => el._id === administrationId);
  if (type) {
    accumalator.reduce((acc, administration) => {
      if (type === 'addManager') {
        if (accumalator[0].management[0].id === id) {
          let lastChild = 0;
          if (administration.management[0].staff[0].subdivisions.length) {
          ...
          } else if (administration.management[0].staff[0].people.length) {
            lastChild = _.last(administration.management[0].staff[0].people)
          }
          const index = findMaxIndex(uIstate, administrationId);
          const user = {
            id: index,
            position : position,
            full_name : fullName,
